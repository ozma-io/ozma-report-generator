...
namespace ReportGenerator.OzmaDBApi
{
    public class OzmaDBQuery
    {
        ...
        public OzmaDBQuery(string name, string queryTextWithoutParameterValues, QueryType queryType)
        {
            Name = name;
        ...
        public async Task LoadDataAsync(OzmaDBApiConnector ozmaDbApiConnector, Dictionary<string, object> queryParametersWithValues)
        {
            ...
            if (queryTextToRun.StartsWith("/views/"))
            {
                ...
                result = await ozmaDbApiConnector.LoadQueryNamed(namedQueryText, queryParametersWithValues);
            }
            else
                result = await ozmaDbApiConnector.LoadQueryAnonymous(queryTextToRun, queryParametersWithValues);
            if (result != null)
            {
                IsLoaded = true;
                ...
                if (viewExprResult?.info != null && viewExprResult.result != null)
                {
                    ...
                    if (QueryType == QueryType.SingleValue)
                    {
                        ...
                        {
                            throw new Exception("OzmaDB query type (SingleValue) does not match returned results");
                        }
                        dynamic? value;
                    ...
                    else if (QueryType == QueryType.SingleRow)
                    {
                        ...
                        {
                            throw new Exception("OzmaDB query type (SingleRow) does not match returned results");
                        }
                    ...
                    {
                        throw new Exception("Wrong OzmaDB query type: " + QueryType);
                    }
                    BarCodeFieldValues = GetBarCodeFieldValues(viewExprResult);
                }
                else
                {
                    throw new Exception("OzmaDB query execution error " + responseJson);
                }
            }
            _result = result;
        }
